...
from utils import clean_tags, get_district_query
...
class EstateInfoService():
  ...
  def __query_by_filter__(self, query_dto):
    ...
    if type(query_dto.get("room_layouts")) is list and len(query_dto["room_layouts"]):
      match_filter["room_layouts"] = { "$all": query_dto["room_layouts"] }
    if type(query_dto.get("estate_tags")) is list and len(query_dto["estate_tags"]):
      match_filter["estate_tags"] = { "$all": query_dto["estate_tags"] }
    if type(query_dto.get("districts")) is list and len(query_dto["districts"]):
      match_filter["$or"] = [
        get_district_query(pairs)
        for pairs in query_dto["districts"]
      ]
  ...
  def create(self, dto, user_id=None):
    ...
    if type(dto.get("estate_tags")) is list:
      dto["estate_tags"] = clean_tags(dto["estate_tags"])
